...
namespace SecurityRemotePasswordProtocol
{
    class Server
    {
        ...
        public Server(string username, byte[] byte_s, byte[] byte_v, byte[] byte_A) : this(username, byte_s, byte_v, byte_A, null, null)
        {
        }
        ...
        public Server(string username, byte[] byte_s, byte[] byte_v, byte[] byte_A, byte[] byte_N, byte[] byte_g)
        {
            // Set stuff
            ...
            using (SHA512 h = SHA512.Create())
            {
                ...
                BigInteger N = byte_N != null ? new BigInteger(byte_N) : Default.N;
                BigInteger g = byte_g != null ? new BigInteger(byte_g) : Default.g;
                ...
                if(!safety_failed)
                {
                    ...
                    byte_M = General.GenerateM(N.getBytes(), g.getBytes(), new ASCIIEncoding().GetBytes(username), byte_s, byte_A, B.getBytes(), byte_K);
